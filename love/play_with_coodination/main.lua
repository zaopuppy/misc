...
function love.draw()
    ...
    -- draw marked grids
    for k, v in pairs(g_marked_pointers) do
        local c, r = v[1], v[2]
...
function love.mousepressed(x, y, button, istouch, presses)
    ...
    if button == 1 then
        -- mark grid
        local idx = find_idx(g_marked_pointers, near_x, near_y)
        if idx < 0 then
            table.insert(g_marked_pointers, {near_x, near_y})
        end
    elseif button == 2 then
        -- unmark grid
        local idx = find_idx(g_marked_pointers, near_x, near_y)
        if idx >= 0 then
            table.remove(g_marked_pointers, idx)
        end
    end
end
